...
// Function with loop
std::vector<int> calculateFactorial(long int baseNum)
{
    std::vector<int> myDigits = {1};

    for (long int i = 2; i <= baseNum; ++i)
    {
        myDigits = multiplyNum(i, myDigits);
    }
...
int main()
{
    ...
    std::vector<int> calcResult = calculateFactorial(bigNum);
